...
folder = "higgsportal" if model == "higgs" else "staus"

file_list = glob.glob("/eos/user/k/kdipetri/Snowmass_HepMC/run_{}/*/events.hepmc".format(folder))
pt_pass_checks = [0.5, 1.0, 2.0, 5.0, 10.0]
d0_pass_checks = [10, 20, 50, 100]
d0_min_check = 1
pt_min_check = 0.5
track_low_cut = ntracks 
max_track_eff = 1
seed(1)
use_slope_eff = slope #change below line if you want binary slope (false) or realistic slope (true)
...
for m in range(len(file_list)):
    ...
    #Event Level Start ==============================================================================================
        ...
        while True :
            ...
            if doTest: 
                if evt.event_number > 100: break #only first 100evts
                if m > 0: break # only first file
...
slope_str="slope" if slope else "Noslope" 
if doTest : model = "test_"+model
save_name = 'output/%s_%dtrack_%.1feffs_%s.json'%(model,track_low_cut,max_track_eff,slope_str)
